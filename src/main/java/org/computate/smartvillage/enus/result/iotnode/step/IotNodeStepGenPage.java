...
public class IotNodeStepGenPage extends IotNodeStepGenPageGen<BaseResultPage> {
	...
	protected void _varsQ(JsonObject vars) {
		IotNodeStep.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListIotNodeStep_.getRequest().getQuery()).filter(fq -> fq.startsWith(IotNodeStep.varIndexedIotNodeStep(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		IotNodeStep.varsFqForClass().forEach(var -> {
			...
			json.put("displayName", Optional.ofNullable(IotNodeStep.displayNameIotNodeStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(IotNodeStep.classSimpleNameIotNodeStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListIotNodeStep_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(IotNodeStep.varIndexedIotNodeStep(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListIotNodeStep_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListIotNodeStep_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListIotNodeStep_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
