...
import org.computate.smartvillage.enus.model.traffic.time.step.TimeStepPage;
...
public class TimeStepEnUSGenApiServiceImpl extends BaseApiServiceImpl implements TimeStepEnUSGenApiService {
	...
	public void searchTimeStep(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
						!Optional.ofNullable(config.getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_TimeStep")).map(v -> RoleBasedAuthorization.create(v).match(siteRequest.getUser())).orElse(false)
						&& !Optional.ofNullable(Optional.ofNullable(config.getString(ConfigKeys.AUTH_ROLE_READ_REQUIRED + "_TimeStep")).orElse(config.getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_TimeStep"))).map(v -> RoleBasedAuthorization.create(v).match(siteRequest.getUser())).orElse(false)
						) {
					String msg = String.format("401 UNAUTHORIZED user %s to %s %s", siteRequest.getUser().attributes().getJsonObject("accessToken").getString("preferred_username"), serviceRequest.getExtra().getString("method"), serviceRequest.getExtra().getString("uri"));
					eventHandler.handle(Future.succeededFuture(
						new ServiceResponse(401, "UNAUTHORIZED",
						...
						searchTimeStepList(siteRequest, true, false, true).onSuccess(listTimeStep -> {
							try {
								if(listTimeStep.getResponse().getResponse().getNumFound() > 1
									...
									String message = String.format("roles required: " + config.getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_TimeStep"));
									LOG.error(message);
			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
					...
					try {
						...
						eventBus.request(TimeStep.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "postTimeStepFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request(TimeStep.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "putimportTimeStepFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
	...
	public void putimportTimeStepFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				apiRequest.initDeepApiRequest(siteRequest);
				siteRequest.setApiRequest_(apiRequest);
				String inheritPk = Optional.ofNullable(body.getString(TimeStep.VAR_id)).orElse(body.getString(TimeStep.VAR_id));
					...
					try {
						if(searchList.size() >= 1) {
							...
							postTimeStepFuture(siteRequest, true).onSuccess(b -> {
								LOG.debug("Import TimeStep {} succeeded, created new TimeStep. ", body.getValue(TimeStep.VAR_id));
								eventHandler.handle(Future.succeededFuture());
							}).onFailure(ex -> {
								LOG.error(String.format("putimportTimeStepFuture failed. "), ex);
								eventHandler.handle(Future.failedFuture(ex));
	...
	public String searchVar(String varIndexed) {
		return TimeStep.searchVarTimeStep(varIndexed);
	}

	@Override
	public String getClassApiAddress() {
		return TimeStep.CLASS_API_ADDRESS_TimeStep;
	}
}
