...
public class VehicleStepGenPage extends VehicleStepGenPageGen<MapResultPage> {
	...
	protected void _varsQ(JsonObject vars) {
		VehicleStep.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListVehicleStep_.getRequest().getQuery()).filter(fq -> fq.startsWith(VehicleStep.varIndexedVehicleStep(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		VehicleStep.varsFqForClass().forEach(var -> {
			...
			json.put("displayName", Optional.ofNullable(VehicleStep.displayNameVehicleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(VehicleStep.classSimpleNameVehicleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListVehicleStep_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(VehicleStep.varIndexedVehicleStep(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListVehicleStep_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListVehicleStep_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListVehicleStep_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListVehicleStep_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return VehicleStep.searchVarVehicleStep(v); }).orElse("created"));
	}
	...
	protected void _roleForWrite(List<String> l) {
		Optional.ofNullable(siteRequest_.getConfig().getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_VehicleStep")).ifPresent(v -> {
			l.add(v);
		});
	}
