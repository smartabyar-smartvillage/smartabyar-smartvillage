 ...
 * Generated: true
 **/
public class PhotovoltaicMeasurementGenPage extends PhotovoltaicMeasurementGenPageGen<BaseModelPage> {

	/**
	 * {@inheritDoc}
	 * Ignore: true
	 **/
	protected void _searchListPhotovoltaicMeasurement_(Wrap<SearchList<PhotovoltaicMeasurement>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListPhotovoltaicMeasurement_ != null)
			w.o(JsonObject.mapFrom(searchListPhotovoltaicMeasurement_.getResponse()).toString());
	}

	@Override
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListPhotovoltaicMeasurement_.getResponse().getStats());
	}

	@Override
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListPhotovoltaicMeasurement_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListPhotovoltaicMeasurement_.getStart().longValue();
		Long rows = searchListPhotovoltaicMeasurement_.getRows().longValue();
		Long foundNum = searchListPhotovoltaicMeasurement_.getResponse().getResponse().getNumFound().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		PhotovoltaicMeasurement.varsQForClass().forEach(var -> {
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(PhotovoltaicMeasurement.displayNamePhotovoltaicMeasurement(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(PhotovoltaicMeasurement.classSimpleNamePhotovoltaicMeasurement(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", Optional.ofNullable(searchListPhotovoltaicMeasurement_.getRequest().getQuery()).filter(fq -> fq.startsWith(PhotovoltaicMeasurement.varIndexedPhotovoltaicMeasurement(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		PhotovoltaicMeasurement.varsFqForClass().forEach(var -> {
			String varIndexed = PhotovoltaicMeasurement.varIndexedPhotovoltaicMeasurement(var);
			String varStored = PhotovoltaicMeasurement.varStoredPhotovoltaicMeasurement(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			...
			json.put("displayName", Optional.ofNullable(PhotovoltaicMeasurement.displayNamePhotovoltaicMeasurement(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(PhotovoltaicMeasurement.classSimpleNamePhotovoltaicMeasurement(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListPhotovoltaicMeasurement_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(PhotovoltaicMeasurement.varIndexedPhotovoltaicMeasurement(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _varsRange(JsonObject vars) {
		PhotovoltaicMeasurement.varsRangeForClass().forEach(var -> {
			String varIndexed = PhotovoltaicMeasurement.varIndexedPhotovoltaicMeasurement(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(PhotovoltaicMeasurement.displayNamePhotovoltaicMeasurement(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListPhotovoltaicMeasurement_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListPhotovoltaicMeasurement_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListPhotovoltaicMeasurement_).map(l -> l.getStart()).orElse(1L);
		...
		for(String fq : Optional.ofNullable(searchListPhotovoltaicMeasurement_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = PhotovoltaicMeasurement.searchVarPhotovoltaicMeasurement(StringUtils.substringBefore(fq, ":"));
				String fq2 = StringUtils.substringAfter(fq, ":");
				if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
					fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", PhotovoltaicMeasurement.displayNameForClass(fq1)));
				}
			}
		query.put("fq", fqs);

		JsonArray sorts = new JsonArray();
		for(String sort : Optional.ofNullable(searchListPhotovoltaicMeasurement_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = PhotovoltaicMeasurement.searchVarPhotovoltaicMeasurement(StringUtils.substringBefore(sort, " "));
			sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", PhotovoltaicMeasurement.displayNameForClass(sort1)));
		}
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return PhotovoltaicMeasurement.searchVarPhotovoltaicMeasurement(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetMinCount()).orElse(1));
	}

	@Override
	protected void _defaultPivotMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetPivotMinCount()).orElse(0));
	}
	...
	protected void _defaultFieldListVars(List<String> l) {
		Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = PhotovoltaicMeasurement.searchVarPhotovoltaicMeasurement(part);
	...
	protected void _defaultStatsVars(List<String> l) {
		Optional.ofNullable(searchListPhotovoltaicMeasurement_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
	...
	protected void _defaultPivotVars(List<String> l) {
		Optional.ofNullable(searchListPhotovoltaicMeasurement_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
	 ...
	 * {@inheritDoc}
	 **/
	protected void _listPhotovoltaicMeasurement(JsonArray l) {
		Optional.ofNullable(searchListPhotovoltaicMeasurement_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
	}

	protected void _photovoltaicMeasurementCount(Wrap<Integer> w) {
		w.o(searchListPhotovoltaicMeasurement_ == null ? 0 : searchListPhotovoltaicMeasurement_.size());
	}

	protected void _photovoltaicMeasurement_(Wrap<PhotovoltaicMeasurement> w) {
		if(photovoltaicMeasurementCount == 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("id")).orElse(null) != null)
			w.o(searchListPhotovoltaicMeasurement_.get(0));
	}

	protected void _pk(Wrap<Long> w) {
		if(photovoltaicMeasurement_ != null)
			w.o(photovoltaicMeasurement_.getPk());
	}

	protected void _id(Wrap<String> w) {
		if(photovoltaicMeasurement_ != null)
			w.o(photovoltaicMeasurement_.getId());
	}
	...
	protected void _classSimpleName(Wrap<String> w) {
		w.o("PhotovoltaicMeasurement");
	}

	@Override
	protected void _pageTitle(Wrap<String> c) {
		if(photovoltaicMeasurement_ != null && photovoltaicMeasurement_.getObjectTitle() != null)
			c.o(photovoltaicMeasurement_.getObjectTitle());
		else if(photovoltaicMeasurement_ != null)
			c.o("photovoltaic measurements");
		else if(searchListPhotovoltaicMeasurement_ == null || photovoltaicMeasurementCount == 0)
			c.o("no photovoltaic measurement found");
		else
			c.o("photovoltaic measurements");
	}
	...
	protected void _pageUri(Wrap<String> c) {
		c.o("/PhotovoltaicMeasurement");
	}

	@Override
	protected void _apiUri(Wrap<String> c) {
		c.o("/api/PhotovoltaicMeasurement");
	}
	...
	protected void _roleForWrite(List<String> l) {
		Optional.ofNullable(siteRequest_.getConfig().getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_PhotovoltaicMeasurement")).ifPresent(v -> {
			l.add(v);
		});
	}
	...
	protected void _pageDescription(Wrap<String> c) {
			c.o("ApiTag.enUS: PhotovoltaicMeasurement");
	}

	@Override
	protected void _pageImageUri(Wrap<String> c) {
			c.o("/png/PhotovoltaicMeasurement-999.png");
	}
	...
	protected void _pageUriPhotovoltaicMeasurement(Wrap<String> c) {
			c.o("/PhotovoltaicMeasurement");
	}
}
