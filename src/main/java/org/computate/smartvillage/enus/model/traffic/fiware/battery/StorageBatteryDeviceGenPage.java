 ...
 * Generated: true
 **/
public class StorageBatteryDeviceGenPage extends StorageBatteryDeviceGenPageGen<BaseModelPage> {

	/**
	 * {@inheritDoc}
	 * Ignore: true
	 **/
	protected void _searchListStorageBatteryDevice_(Wrap<SearchList<StorageBatteryDevice>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListStorageBatteryDevice_ != null)
			w.o(JsonObject.mapFrom(searchListStorageBatteryDevice_.getResponse()).toString());
	}

	@Override
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListStorageBatteryDevice_.getResponse().getStats());
	}

	@Override
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListStorageBatteryDevice_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListStorageBatteryDevice_.getStart().longValue();
		Long rows = searchListStorageBatteryDevice_.getRows().longValue();
		Long foundNum = searchListStorageBatteryDevice_.getResponse().getResponse().getNumFound().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		StorageBatteryDevice.varsQForClass().forEach(var -> {
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(StorageBatteryDevice.displayNameStorageBatteryDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(StorageBatteryDevice.classSimpleNameStorageBatteryDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", Optional.ofNullable(searchListStorageBatteryDevice_.getRequest().getQuery()).filter(fq -> fq.startsWith(StorageBatteryDevice.varIndexedStorageBatteryDevice(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		StorageBatteryDevice.varsFqForClass().forEach(var -> {
			String varIndexed = StorageBatteryDevice.varIndexedStorageBatteryDevice(var);
			String varStored = StorageBatteryDevice.varStoredStorageBatteryDevice(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			...
			json.put("displayName", Optional.ofNullable(StorageBatteryDevice.displayNameStorageBatteryDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(StorageBatteryDevice.classSimpleNameStorageBatteryDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListStorageBatteryDevice_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(StorageBatteryDevice.varIndexedStorageBatteryDevice(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _varsRange(JsonObject vars) {
		StorageBatteryDevice.varsRangeForClass().forEach(var -> {
			String varIndexed = StorageBatteryDevice.varIndexedStorageBatteryDevice(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(StorageBatteryDevice.displayNameStorageBatteryDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListStorageBatteryDevice_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListStorageBatteryDevice_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListStorageBatteryDevice_).map(l -> l.getStart()).orElse(1L);
		...
		for(String fq : Optional.ofNullable(searchListStorageBatteryDevice_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = StorageBatteryDevice.searchVarStorageBatteryDevice(StringUtils.substringBefore(fq, ":"));
				String fq2 = StringUtils.substringAfter(fq, ":");
				if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
					fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", StorageBatteryDevice.displayNameForClass(fq1)));
				}
			}
		query.put("fq", fqs);

		JsonArray sorts = new JsonArray();
		for(String sort : Optional.ofNullable(searchListStorageBatteryDevice_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = StorageBatteryDevice.searchVarStorageBatteryDevice(StringUtils.substringBefore(sort, " "));
			sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", StorageBatteryDevice.displayNameForClass(sort1)));
		}
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListStorageBatteryDevice_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListStorageBatteryDevice_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListStorageBatteryDevice_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListStorageBatteryDevice_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return StorageBatteryDevice.searchVarStorageBatteryDevice(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListStorageBatteryDevice_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListStorageBatteryDevice_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListStorageBatteryDevice_.getFacetMinCount()).orElse(1));
	}

	@Override
	protected void _defaultPivotMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListStorageBatteryDevice_.getFacetPivotMinCount()).orElse(0));
	}
	...
	protected void _defaultFieldListVars(List<String> l) {
		Optional.ofNullable(searchListStorageBatteryDevice_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = StorageBatteryDevice.searchVarStorageBatteryDevice(part);
	...
	protected void _defaultStatsVars(List<String> l) {
		Optional.ofNullable(searchListStorageBatteryDevice_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
	...
	protected void _defaultPivotVars(List<String> l) {
		Optional.ofNullable(searchListStorageBatteryDevice_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
	 ...
	 * {@inheritDoc}
	 **/
	protected void _listStorageBatteryDevice(JsonArray l) {
		Optional.ofNullable(searchListStorageBatteryDevice_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
	}

	protected void _storageBatteryDeviceCount(Wrap<Integer> w) {
		w.o(searchListStorageBatteryDevice_ == null ? 0 : searchListStorageBatteryDevice_.size());
	}

	protected void _storageBatteryDevice_(Wrap<StorageBatteryDevice> w) {
		if(storageBatteryDeviceCount == 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("id")).orElse(null) != null)
			w.o(searchListStorageBatteryDevice_.get(0));
	}

	protected void _pk(Wrap<Long> w) {
		if(storageBatteryDevice_ != null)
			w.o(storageBatteryDevice_.getPk());
	}

	protected void _id(Wrap<String> w) {
		if(storageBatteryDevice_ != null)
			w.o(storageBatteryDevice_.getId());
	}
	...
	protected void _classSimpleName(Wrap<String> w) {
		w.o("StorageBatteryDevice");
	}

	@Override
	protected void _pageTitle(Wrap<String> c) {
		if(storageBatteryDevice_ != null && storageBatteryDevice_.getObjectTitle() != null)
			c.o(storageBatteryDevice_.getObjectTitle());
		else if(storageBatteryDevice_ != null)
			c.o("StorageBatteryDevices");
		else if(searchListStorageBatteryDevice_ == null || storageBatteryDeviceCount == 0)
			c.o("no StorageBatteryDevice found");
		else
			c.o("StorageBatteryDevices");
	}
	...
	protected void _pageUri(Wrap<String> c) {
		c.o("/StorageBatteryDevice");
	}

	@Override
	protected void _apiUri(Wrap<String> c) {
		c.o("/api/StorageBatteryDevice");
	}
	...
	protected void _roleForWrite(List<String> l) {
		Optional.ofNullable(siteRequest_.getConfig().getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_StorageBatteryDevice")).ifPresent(v -> {
			l.add(v);
		});
	}
	...
	protected void _pageDescription(Wrap<String> c) {
			c.o("ApiTag.enUS: StorageBatteryDevice");
	}

	@Override
	protected void _pageImageUri(Wrap<String> c) {
			c.o("/png/StorageBatteryDevice-999.png");
	}
	...
	protected void _pageUriStorageBatteryDevice(Wrap<String> c) {
			c.o("/StorageBatteryDevice");
	}
}
