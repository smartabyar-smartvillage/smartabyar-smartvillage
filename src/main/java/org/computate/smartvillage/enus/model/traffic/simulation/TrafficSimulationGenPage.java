 ...
 * Generated: true
 **/
public class TrafficSimulationGenPage extends TrafficSimulationGenPageGen<BaseModelPage> {
	...
	protected void _varsQ(JsonObject vars) {
		TrafficSimulation.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListTrafficSimulation_.getRequest().getQuery()).filter(fq -> fq.startsWith(TrafficSimulation.varIndexedTrafficSimulation(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		TrafficSimulation.varsFqForClass().forEach(var -> {
			...
			json.put("displayName", Optional.ofNullable(TrafficSimulation.displayNameTrafficSimulation(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(TrafficSimulation.classSimpleNameTrafficSimulation(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListTrafficSimulation_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TrafficSimulation.varIndexedTrafficSimulation(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListTrafficSimulation_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListTrafficSimulation_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListTrafficSimulation_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListTrafficSimulation_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return TrafficSimulation.searchVarTrafficSimulation(v); }).orElse("created"));
	}
