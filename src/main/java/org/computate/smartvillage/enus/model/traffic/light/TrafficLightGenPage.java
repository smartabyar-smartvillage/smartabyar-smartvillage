...
public class TrafficLightGenPage extends TrafficLightGenPageGen<MapResultPage> {
	...
	protected void _varsQ(JsonObject vars) {
		TrafficLight.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListTrafficLight_.getRequest().getQuery()).filter(fq -> fq.startsWith(TrafficLight.varIndexedTrafficLight(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		TrafficLight.varsFqForClass().forEach(var -> {
			...
			json.put("displayName", Optional.ofNullable(TrafficLight.displayNameTrafficLight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(TrafficLight.classSimpleNameTrafficLight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListTrafficLight_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TrafficLight.varIndexedTrafficLight(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _trafficLight_(Wrap<TrafficLight> w) {
		if(trafficLightCount == 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("id")).orElse(null) != null)
