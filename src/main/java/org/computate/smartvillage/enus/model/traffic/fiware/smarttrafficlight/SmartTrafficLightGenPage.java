...
public class SmartTrafficLightGenPage extends SmartTrafficLightGenPageGen<BaseModelPage> {
	 ...
	 * Ignore: true
	 **/
	protected void _searchListSmartTrafficLight_(Wrap<SearchList<SmartTrafficLight>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListSmartTrafficLight_ != null)
			w.o(JsonObject.mapFrom(searchListSmartTrafficLight_.getResponse()).toString());
	}

	@Override
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListSmartTrafficLight_.getResponse().getStats());
	}

	@Override
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListSmartTrafficLight_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListSmartTrafficLight_.getStart().longValue();
		Long rows = searchListSmartTrafficLight_.getRows().longValue();
		Long foundNum = searchListSmartTrafficLight_.getResponse().getResponse().getNumFound().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		SmartTrafficLight.varsQForClass().forEach(var -> {
	...
	protected void _varsFq(JsonObject vars) {
		...
		SmartTrafficLight.varsFqForClass().forEach(var -> {
			String varIndexed = SmartTrafficLight.varIndexedSmartTrafficLight(var);
			String varStored = SmartTrafficLight.varStoredSmartTrafficLight(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
	...
	protected void _varsRange(JsonObject vars) {
		SmartTrafficLight.varsRangeForClass().forEach(var -> {
			String varIndexed = SmartTrafficLight.varIndexedSmartTrafficLight(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(SmartTrafficLight.displayNameSmartTrafficLight(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListSmartTrafficLight_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListSmartTrafficLight_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListSmartTrafficLight_).map(l -> l.getStart()).orElse(1L);
		...
		for(String fq : Optional.ofNullable(searchListSmartTrafficLight_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = SmartTrafficLight.searchVarSmartTrafficLight(StringUtils.substringBefore(fq, ":"));
				String fq2 = StringUtils.substringAfter(fq, ":");
				if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
					fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", SmartTrafficLight.displayNameForClass(fq1)));
				}
			}
		query.put("fq", fqs);

		JsonArray sorts = new JsonArray();
		for(String sort : Optional.ofNullable(searchListSmartTrafficLight_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = SmartTrafficLight.searchVarSmartTrafficLight(StringUtils.substringBefore(sort, " "));
			sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", SmartTrafficLight.displayNameForClass(sort1)));
		}
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListSmartTrafficLight_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListSmartTrafficLight_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListSmartTrafficLight_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListSmartTrafficLight_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return SmartTrafficLight.searchVarSmartTrafficLight(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListSmartTrafficLight_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListSmartTrafficLight_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListSmartTrafficLight_.getFacetMinCount()).orElse(1));
	}

	@Override
	protected void _defaultPivotMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListSmartTrafficLight_.getFacetPivotMinCount()).orElse(0));
	}
	...
	protected void _defaultFieldListVars(List<String> l) {
		Optional.ofNullable(searchListSmartTrafficLight_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = SmartTrafficLight.searchVarSmartTrafficLight(part);
	...
	protected void _defaultStatsVars(List<String> l) {
		Optional.ofNullable(searchListSmartTrafficLight_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
	...
	protected void _defaultPivotVars(List<String> l) {
		Optional.ofNullable(searchListSmartTrafficLight_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
	 ...
	 * {@inheritDoc}
	 **/
	protected void _listSmartTrafficLight(JsonArray l) {
		Optional.ofNullable(searchListSmartTrafficLight_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
	}

	protected void _smartTrafficLightCount(Wrap<Integer> w) {
		w.o(searchListSmartTrafficLight_ == null ? 0 : searchListSmartTrafficLight_.size());
	}
	...
	protected void _classSimpleName(Wrap<String> w) {
		w.o("SmartTrafficLight");
	}

	@Override
	protected void _pageTitle(Wrap<String> c) {
		if(smartTrafficLight_ != null && smartTrafficLight_.getObjectTitle() != null)
			c.o(smartTrafficLight_.getObjectTitle());
		else if(smartTrafficLight_ != null)
			c.o("smart traffic lights");
		else if(searchListSmartTrafficLight_ == null || smartTrafficLightCount == 0)
			c.o("no smart traffic light found");
		else
			c.o("smart traffic lights");
	}
	...
	protected void _pageUri(Wrap<String> c) {
		c.o("/smart-traffic-light");
	}

	@Override
	protected void _apiUri(Wrap<String> c) {
		c.o("/api/smart-traffic-light");
	}
	...
	protected void _pageImageUri(Wrap<String> c) {
			c.o("/png/smart-traffic-light-999.png");
	}
	...
	protected void _pageUriSmartTrafficLight(Wrap<String> c) {
			c.o("/smart-traffic-light");
	}
}
