 ...
 * Generated: true
 **/
public class BicycleStepGenPage extends BicycleStepGenPageGen<MapResultPage> {

	/**
	 * {@inheritDoc}
	 * Ignore: true
	 **/
	protected void _searchListBicycleStep_(Wrap<SearchList<BicycleStep>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListBicycleStep_ != null)
			w.o(JsonObject.mapFrom(searchListBicycleStep_.getResponse()).toString());
	}

	@Override
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListBicycleStep_.getResponse().getStats());
	}

	@Override
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListBicycleStep_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListBicycleStep_.getStart().longValue();
		Long rows = searchListBicycleStep_.getRows().longValue();
		Long foundNum = searchListBicycleStep_.getResponse().getResponse().getNumFound().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		BicycleStep.varsQForClass().forEach(var -> {
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(BicycleStep.displayNameBicycleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(BicycleStep.classSimpleNameBicycleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", Optional.ofNullable(searchListBicycleStep_.getRequest().getQuery()).filter(fq -> fq.startsWith(BicycleStep.varIndexedBicycleStep(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		BicycleStep.varsFqForClass().forEach(var -> {
			String varIndexed = BicycleStep.varIndexedBicycleStep(var);
			String varStored = BicycleStep.varStoredBicycleStep(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			...
			json.put("displayName", Optional.ofNullable(BicycleStep.displayNameBicycleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(BicycleStep.classSimpleNameBicycleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListBicycleStep_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(BicycleStep.varIndexedBicycleStep(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _varsRange(JsonObject vars) {
		BicycleStep.varsRangeForClass().forEach(var -> {
			String varIndexed = BicycleStep.varIndexedBicycleStep(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(BicycleStep.displayNameBicycleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListBicycleStep_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListBicycleStep_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListBicycleStep_).map(l -> l.getStart()).orElse(1L);
		...
		for(String fq : Optional.ofNullable(searchListBicycleStep_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = BicycleStep.searchVarBicycleStep(StringUtils.substringBefore(fq, ":"));
				String fq2 = StringUtils.substringAfter(fq, ":");
				if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
					fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", BicycleStep.displayNameForClass(fq1)));
				}
			}
		query.put("fq", fqs);

		JsonArray sorts = new JsonArray();
		for(String sort : Optional.ofNullable(searchListBicycleStep_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = BicycleStep.searchVarBicycleStep(StringUtils.substringBefore(sort, " "));
			sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", BicycleStep.displayNameForClass(sort1)));
		}
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListBicycleStep_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListBicycleStep_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListBicycleStep_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListBicycleStep_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return BicycleStep.searchVarBicycleStep(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListBicycleStep_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListBicycleStep_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListBicycleStep_.getFacetMinCount()).orElse(1));
	}

	@Override
	protected void _defaultPivotMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListBicycleStep_.getFacetPivotMinCount()).orElse(0));
	}
	...
	protected void _defaultFieldListVars(List<String> l) {
		Optional.ofNullable(searchListBicycleStep_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = BicycleStep.searchVarBicycleStep(part);
	...
	protected void _defaultStatsVars(List<String> l) {
		Optional.ofNullable(searchListBicycleStep_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
	...
	protected void _defaultPivotVars(List<String> l) {
		Optional.ofNullable(searchListBicycleStep_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
	 ...
	 * {@inheritDoc}
	 **/
	protected void _listBicycleStep(JsonArray l) {
		Optional.ofNullable(searchListBicycleStep_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
	}

	protected void _bicycleStepCount(Wrap<Integer> w) {
		w.o(searchListBicycleStep_ == null ? 0 : searchListBicycleStep_.size());
	}
	...
	protected void _pageTitle(Wrap<String> c) {
		if(bicycleStep_ != null && bicycleStep_.getObjectTitle() != null)
			c.o(bicycleStep_.getObjectTitle());
		else if(bicycleStep_ != null)
			c.o("bicycle steps");
		else if(searchListBicycleStep_ == null || bicycleStepCount == 0)
			c.o("no bicycle step found");
		else
			c.o("bicycle steps");
	}
	...
	protected void _pageUriBicycleStep(Wrap<String> c) {
			c.o("/bicycle-step");
	}
}
