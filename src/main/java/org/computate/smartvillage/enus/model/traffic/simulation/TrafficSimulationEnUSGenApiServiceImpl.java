...
import org.computate.smartvillage.enus.model.traffic.simulation.report.SimulationReportEnUSApiServiceImpl;
...
import org.computate.smartvillage.enus.model.traffic.simulation.TrafficSimulationPage;
import org.computate.smartvillage.enus.model.traffic.simulation.map.TrafficSimulationMapPage;
...
public class TrafficSimulationEnUSGenApiServiceImpl extends BaseApiServiceImpl implements TrafficSimulationEnUSGenApiService {
			...
			for(String entityVar : methodNames) {
				switch(entityVar) {
					...
					case "setEntityId":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(TrafficSimulation.VAR_entityId + "=$" + num);
							num++;
			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
					...
					try {
						...
						eventBus.request(TrafficSimulation.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "postTrafficSimulationFuture")).onSuccess(a -> {
			...
			if(jsonObject != null) {
				...
				for(String entityVar : entityVars) {
					...
					case TrafficSimulation.VAR_simulationName:
						...
						bParams.add(o2.sqlSimulationName());
						break;
					case TrafficSimulation.VAR_entityId:
						o2.setEntityId(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request(TrafficSimulation.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "putimportTrafficSimulationFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
	...
	public void putimportTrafficSimulationFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				apiRequest.initDeepApiRequest(siteRequest);
				siteRequest.setApiRequest_(apiRequest);
				String inheritPk = Optional.ofNullable(body.getString(TrafficSimulation.VAR_pk)).orElse(body.getString(TrafficSimulation.VAR_id));
	...
	public String getClassApiAddress() {
		return TrafficSimulation.CLASS_API_ADDRESS_TrafficSimulation;
	}
		...
		try {
			...
			if(refresh && !Optional.ofNullable(siteRequest.getJsonObject()).map(JsonObject::isEmpty).orElse(true)) {
				...
				CompositeFuture.all(futures).onSuccess(b -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request(TrafficSimulation.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "patchTrafficSimulationFuture")).onSuccess(c -> {
						JsonObject responseMessage = (JsonObject)c.body();
						Integer statusCode = responseMessage.getInteger("statusCode");
						if(statusCode.equals(200))
							promise.complete();
