...
public class TrafficFlowObservedGenPage extends TrafficFlowObservedGenPageGen<BaseModelPage> {
	 ...
	 * Ignore: true
	 **/
	protected void _searchListTrafficFlowObserved_(Wrap<SearchList<TrafficFlowObserved>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListTrafficFlowObserved_ != null)
			w.o(JsonObject.mapFrom(searchListTrafficFlowObserved_.getResponse()).toString());
	}

	@Override
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListTrafficFlowObserved_.getResponse().getStats());
	}

	@Override
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListTrafficFlowObserved_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListTrafficFlowObserved_.getStart().longValue();
		Long rows = searchListTrafficFlowObserved_.getRows().longValue();
		Long foundNum = searchListTrafficFlowObserved_.getResponse().getResponse().getNumFound().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		TrafficFlowObserved.varsQForClass().forEach(var -> {
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(TrafficFlowObserved.displayNameTrafficFlowObserved(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(TrafficFlowObserved.classSimpleNameTrafficFlowObserved(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", Optional.ofNullable(searchListTrafficFlowObserved_.getRequest().getQuery()).filter(fq -> fq.startsWith(TrafficFlowObserved.varIndexedTrafficFlowObserved(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		TrafficFlowObserved.varsFqForClass().forEach(var -> {
			String varIndexed = TrafficFlowObserved.varIndexedTrafficFlowObserved(var);
			String varStored = TrafficFlowObserved.varStoredTrafficFlowObserved(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			...
			json.put("displayName", Optional.ofNullable(TrafficFlowObserved.displayNameTrafficFlowObserved(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(TrafficFlowObserved.classSimpleNameTrafficFlowObserved(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListTrafficFlowObserved_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TrafficFlowObserved.varIndexedTrafficFlowObserved(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _varsRange(JsonObject vars) {
		TrafficFlowObserved.varsRangeForClass().forEach(var -> {
			String varIndexed = TrafficFlowObserved.varIndexedTrafficFlowObserved(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(TrafficFlowObserved.displayNameTrafficFlowObserved(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListTrafficFlowObserved_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListTrafficFlowObserved_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListTrafficFlowObserved_).map(l -> l.getStart()).orElse(1L);
		...
		for(String fq : Optional.ofNullable(searchListTrafficFlowObserved_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = TrafficFlowObserved.searchVarTrafficFlowObserved(StringUtils.substringBefore(fq, ":"));
				String fq2 = StringUtils.substringAfter(fq, ":");
				if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
					fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", TrafficFlowObserved.displayNameForClass(fq1)));
				}
			}
		query.put("fq", fqs);

		JsonArray sorts = new JsonArray();
		for(String sort : Optional.ofNullable(searchListTrafficFlowObserved_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = TrafficFlowObserved.searchVarTrafficFlowObserved(StringUtils.substringBefore(sort, " "));
			sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", TrafficFlowObserved.displayNameForClass(sort1)));
		}
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListTrafficFlowObserved_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListTrafficFlowObserved_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListTrafficFlowObserved_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListTrafficFlowObserved_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return TrafficFlowObserved.searchVarTrafficFlowObserved(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListTrafficFlowObserved_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListTrafficFlowObserved_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListTrafficFlowObserved_.getFacetMinCount()).orElse(1));
	}

	@Override
	protected void _defaultPivotMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListTrafficFlowObserved_.getFacetPivotMinCount()).orElse(0));
	}
	...
	protected void _defaultFieldListVars(List<String> l) {
		Optional.ofNullable(searchListTrafficFlowObserved_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = TrafficFlowObserved.searchVarTrafficFlowObserved(part);
	...
	protected void _defaultStatsVars(List<String> l) {
		Optional.ofNullable(searchListTrafficFlowObserved_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
	...
	protected void _defaultPivotVars(List<String> l) {
		Optional.ofNullable(searchListTrafficFlowObserved_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
	 ...
	 * {@inheritDoc}
	 **/
	protected void _listTrafficFlowObserved(JsonArray l) {
		Optional.ofNullable(searchListTrafficFlowObserved_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
	}

	protected void _trafficFlowObservedCount(Wrap<Integer> w) {
		w.o(searchListTrafficFlowObserved_ == null ? 0 : searchListTrafficFlowObserved_.size());
	}
	...
	protected void _classSimpleName(Wrap<String> w) {
		w.o("TrafficFlowObserved");
	}

	@Override
	protected void _pageTitle(Wrap<String> c) {
		if(trafficFlowObserved_ != null && trafficFlowObserved_.getObjectTitle() != null)
			c.o(trafficFlowObserved_.getObjectTitle());
		else if(trafficFlowObserved_ != null)
			c.o("traffic flow observeds");
		else if(searchListTrafficFlowObserved_ == null || trafficFlowObservedCount == 0)
			c.o("no traffic flow observed found");
		else
			c.o("traffic flow observeds");
	}
	...
	protected void _pageUri(Wrap<String> c) {
		c.o("/traffic-flow-observed");
	}

	@Override
	protected void _apiUri(Wrap<String> c) {
		c.o("/api/traffic-flow-observed");
	}
	...
	protected void _pageUriTrafficFlowObserved(Wrap<String> c) {
			c.o("/traffic-flow-observed");
	}
}
