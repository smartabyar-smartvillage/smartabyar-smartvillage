 ...
 * Generated: true
 **/
public class ParkingAccessGenPage extends ParkingAccessGenPageGen<BaseModelPage> {

	/**
	 * {@inheritDoc}
	 * Ignore: true
	 **/
	protected void _searchListParkingAccess_(Wrap<SearchList<ParkingAccess>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListParkingAccess_ != null)
			w.o(JsonObject.mapFrom(searchListParkingAccess_.getResponse()).toString());
	}

	@Override
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListParkingAccess_.getResponse().getStats());
	}

	@Override
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListParkingAccess_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListParkingAccess_.getStart().longValue();
		Long rows = searchListParkingAccess_.getRows().longValue();
		Long foundNum = searchListParkingAccess_.getResponse().getResponse().getNumFound().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		ParkingAccess.varsQForClass().forEach(var -> {
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(ParkingAccess.displayNameParkingAccess(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(ParkingAccess.classSimpleNameParkingAccess(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", Optional.ofNullable(searchListParkingAccess_.getRequest().getQuery()).filter(fq -> fq.startsWith(ParkingAccess.varIndexedParkingAccess(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		ParkingAccess.varsFqForClass().forEach(var -> {
			String varIndexed = ParkingAccess.varIndexedParkingAccess(var);
			String varStored = ParkingAccess.varStoredParkingAccess(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			...
			json.put("displayName", Optional.ofNullable(ParkingAccess.displayNameParkingAccess(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(ParkingAccess.classSimpleNameParkingAccess(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListParkingAccess_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(ParkingAccess.varIndexedParkingAccess(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _varsRange(JsonObject vars) {
		ParkingAccess.varsRangeForClass().forEach(var -> {
			String varIndexed = ParkingAccess.varIndexedParkingAccess(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(ParkingAccess.displayNameParkingAccess(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListParkingAccess_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListParkingAccess_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListParkingAccess_).map(l -> l.getStart()).orElse(1L);
		...
		for(String fq : Optional.ofNullable(searchListParkingAccess_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = ParkingAccess.searchVarParkingAccess(StringUtils.substringBefore(fq, ":"));
				String fq2 = StringUtils.substringAfter(fq, ":");
				if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
					fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", ParkingAccess.displayNameForClass(fq1)));
				}
			}
		query.put("fq", fqs);

		JsonArray sorts = new JsonArray();
		for(String sort : Optional.ofNullable(searchListParkingAccess_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = ParkingAccess.searchVarParkingAccess(StringUtils.substringBefore(sort, " "));
			sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", ParkingAccess.displayNameForClass(sort1)));
		}
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListParkingAccess_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListParkingAccess_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListParkingAccess_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListParkingAccess_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return ParkingAccess.searchVarParkingAccess(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListParkingAccess_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListParkingAccess_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListParkingAccess_.getFacetMinCount()).orElse(1));
	}

	@Override
	protected void _defaultPivotMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListParkingAccess_.getFacetPivotMinCount()).orElse(0));
	}
	...
	protected void _defaultFieldListVars(List<String> l) {
		Optional.ofNullable(searchListParkingAccess_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = ParkingAccess.searchVarParkingAccess(part);
	...
	protected void _defaultStatsVars(List<String> l) {
		Optional.ofNullable(searchListParkingAccess_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
	...
	protected void _defaultPivotVars(List<String> l) {
		Optional.ofNullable(searchListParkingAccess_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
	 ...
	 * {@inheritDoc}
	 **/
	protected void _listParkingAccess(JsonArray l) {
		Optional.ofNullable(searchListParkingAccess_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
	}

	protected void _parkingAccessCount(Wrap<Integer> w) {
		w.o(searchListParkingAccess_ == null ? 0 : searchListParkingAccess_.size());
	}

	protected void _parkingAccess_(Wrap<ParkingAccess> w) {
		if(parkingAccessCount == 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("id")).orElse(null) != null)
			w.o(searchListParkingAccess_.get(0));
	}

	protected void _pk(Wrap<Long> w) {
		if(parkingAccess_ != null)
			w.o(parkingAccess_.getPk());
	}

	protected void _id(Wrap<String> w) {
		if(parkingAccess_ != null)
			w.o(parkingAccess_.getId());
	}
	...
	protected void _pageTitle(Wrap<String> c) {
		if(parkingAccess_ != null && parkingAccess_.getObjectTitle() != null)
			c.o(parkingAccess_.getObjectTitle());
		else if(parkingAccess_ != null)
			c.o("ParkingAccesss");
		else if(searchListParkingAccess_ == null || parkingAccessCount == 0)
			c.o("no ParkingAccess found");
		else
			c.o("ParkingAccesss");
	}
	...
	protected void _roleForWrite(List<String> l) {
		Optional.ofNullable(siteRequest_.getConfig().getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_ParkingAccess")).ifPresent(v -> {
			l.add(v);
		});
	}
	...
	protected void _pageDescription(Wrap<String> c) {
			c.o("ApiTag.enUS: ParkingAccess");
	}

	@Override
	protected void _pageImageUri(Wrap<String> c) {
			c.o("/png/parking-access-999.png");
	}
	...
	protected void _pageUriParkingAccess(Wrap<String> c) {
			c.o("/parking-access");
	}
}
