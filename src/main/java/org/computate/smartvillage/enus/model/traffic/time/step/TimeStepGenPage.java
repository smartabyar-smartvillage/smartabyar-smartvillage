...
public class TimeStepGenPage extends TimeStepGenPageGen<BaseResultPage> {
	 ...
	 * Ignore: true
	 **/
	protected void _searchListTimeStep_(Wrap<SearchList<TimeStep>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListTimeStep_ != null)
			w.o(JsonObject.mapFrom(searchListTimeStep_.getResponse()).toString());
	}

	@Override
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListTimeStep_.getResponse().getStats());
	}

	@Override
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListTimeStep_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListTimeStep_.getStart().longValue();
		Long rows = searchListTimeStep_.getRows().longValue();
		Long foundNum = searchListTimeStep_.getResponse().getResponse().getNumFound().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		TimeStep.varsQForClass().forEach(var -> {
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(TimeStep.displayNameTimeStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(TimeStep.classSimpleNameTimeStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", Optional.ofNullable(searchListTimeStep_.getRequest().getQuery()).filter(fq -> fq.startsWith(TimeStep.varIndexedTimeStep(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		TimeStep.varsFqForClass().forEach(var -> {
			String varIndexed = TimeStep.varIndexedTimeStep(var);
			String varStored = TimeStep.varStoredTimeStep(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			...
			json.put("displayName", Optional.ofNullable(TimeStep.displayNameTimeStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(TimeStep.classSimpleNameTimeStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListTimeStep_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TimeStep.varIndexedTimeStep(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _varsRange(JsonObject vars) {
		TimeStep.varsRangeForClass().forEach(var -> {
			String varIndexed = TimeStep.varIndexedTimeStep(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(TimeStep.displayNameTimeStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListTimeStep_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListTimeStep_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListTimeStep_).map(l -> l.getStart()).orElse(1L);
		...
		for(String fq : Optional.ofNullable(searchListTimeStep_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = TimeStep.searchVarTimeStep(StringUtils.substringBefore(fq, ":"));
				String fq2 = StringUtils.substringAfter(fq, ":");
				if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
					fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", TimeStep.displayNameForClass(fq1)));
				}
			}
		query.put("fq", fqs);

		JsonArray sorts = new JsonArray();
		for(String sort : Optional.ofNullable(searchListTimeStep_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = TimeStep.searchVarTimeStep(StringUtils.substringBefore(sort, " "));
			sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", TimeStep.displayNameForClass(sort1)));
		}
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListTimeStep_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListTimeStep_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListTimeStep_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListTimeStep_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return TimeStep.searchVarTimeStep(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListTimeStep_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListTimeStep_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListTimeStep_.getFacetMinCount()).orElse(1));
	}

	@Override
	protected void _defaultPivotMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListTimeStep_.getFacetPivotMinCount()).orElse(0));
	}
		...
		Optional.ofNullable(searchListTimeStep_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = TimeStep.searchVarTimeStep(part);
	 ...
	 * {@inheritDoc}
	 **/
	protected void _listTimeStep(JsonArray l) {
		Optional.ofNullable(searchListTimeStep_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
	}

	protected void _timeStepCount(Wrap<Integer> w) {
		w.o(searchListTimeStep_ == null ? 0 : searchListTimeStep_.size());
	}
	...
	protected void _pageTitle(Wrap<String> c) {
		if(timeStep_ != null && timeStep_.getObjectTitle() != null)
			c.o(timeStep_.getObjectTitle());
		else if(timeStep_ != null)
			c.o("time steps");
		else if(searchListTimeStep_ == null || timeStepCount == 0)
			c.o("no time step found");
		else
			c.o("time steps");
	}
	...
	protected void _roleForWrite(List<String> l) {
		Optional.ofNullable(siteRequest_.getConfig().getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_TimeStep")).ifPresent(v -> {
			l.add(v);
		});
	}
