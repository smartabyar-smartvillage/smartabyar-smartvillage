...
public class SimulationReportGenPage extends SimulationReportGenPageGen<BaseModelPage> {
	 ...
	 * Ignore: true
	 **/
	protected void _searchListSimulationReport_(Wrap<SearchList<SimulationReport>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListSimulationReport_ != null)
			w.o(JsonObject.mapFrom(searchListSimulationReport_.getResponse()).toString());
	}

	@Override
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListSimulationReport_.getResponse().getStats());
	}

	@Override
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListSimulationReport_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListSimulationReport_.getStart().longValue();
		Long rows = searchListSimulationReport_.getRows().longValue();
		Long foundNum = searchListSimulationReport_.getResponse().getResponse().getNumFound().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		SimulationReport.varsQForClass().forEach(var -> {
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(SimulationReport.displayNameSimulationReport(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(SimulationReport.classSimpleNameSimulationReport(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", Optional.ofNullable(searchListSimulationReport_.getRequest().getQuery()).filter(fq -> fq.startsWith(SimulationReport.varIndexedSimulationReport(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		SimulationReport.varsFqForClass().forEach(var -> {
			String varIndexed = SimulationReport.varIndexedSimulationReport(var);
			String varStored = SimulationReport.varStoredSimulationReport(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			...
			json.put("displayName", Optional.ofNullable(SimulationReport.displayNameSimulationReport(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(SimulationReport.classSimpleNameSimulationReport(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListSimulationReport_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(SimulationReport.varIndexedSimulationReport(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
	...
	protected void _varsRange(JsonObject vars) {
		SimulationReport.varsRangeForClass().forEach(var -> {
			String varIndexed = SimulationReport.varIndexedSimulationReport(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(SimulationReport.displayNameSimulationReport(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListSimulationReport_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListSimulationReport_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListSimulationReport_).map(l -> l.getStart()).orElse(1L);
		...
		for(String fq : Optional.ofNullable(searchListSimulationReport_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = SimulationReport.searchVarSimulationReport(StringUtils.substringBefore(fq, ":"));
				String fq2 = StringUtils.substringAfter(fq, ":");
				if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
					fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", SimulationReport.displayNameForClass(fq1)));
				}
			}
		query.put("fq", fqs);

		JsonArray sorts = new JsonArray();
		for(String sort : Optional.ofNullable(searchListSimulationReport_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = SimulationReport.searchVarSimulationReport(StringUtils.substringBefore(sort, " "));
			sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", SimulationReport.displayNameForClass(sort1)));
		}
	...
	protected void _rangeGap(Wrap<String> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
			w.o(Optional.ofNullable(searchListSimulationReport_.getFacetRangeGap()).orElse(null));
	}
	...
	protected void _rangeEnd(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
			w.o(Optional.ofNullable(searchListSimulationReport_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _rangeStart(Wrap<ZonedDateTime> w) {
		if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
			w.o(Optional.ofNullable(searchListSimulationReport_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
	}
	...
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListSimulationReport_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return SimulationReport.searchVarSimulationReport(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListSimulationReport_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListSimulationReport_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListSimulationReport_.getFacetMinCount()).orElse(1));
	}

	@Override
	protected void _defaultPivotMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListSimulationReport_.getFacetPivotMinCount()).orElse(0));
	}
	...
	protected void _defaultFieldListVars(List<String> l) {
		Optional.ofNullable(searchListSimulationReport_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = SimulationReport.searchVarSimulationReport(part);
	...
	protected void _defaultStatsVars(List<String> l) {
		Optional.ofNullable(searchListSimulationReport_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
	...
	protected void _defaultPivotVars(List<String> l) {
		Optional.ofNullable(searchListSimulationReport_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
	 ...
	 * {@inheritDoc}
	 **/
	protected void _listSimulationReport(JsonArray l) {
		Optional.ofNullable(searchListSimulationReport_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
	}

	protected void _simulationReportCount(Wrap<Integer> w) {
		w.o(searchListSimulationReport_ == null ? 0 : searchListSimulationReport_.size());
	}
	...
	protected void _classSimpleName(Wrap<String> w) {
		w.o("SimulationReport");
	}

	@Override
	protected void _pageTitle(Wrap<String> c) {
		if(simulationReport_ != null && simulationReport_.getObjectTitle() != null)
			c.o(simulationReport_.getObjectTitle());
		else if(simulationReport_ != null)
			c.o("simulation reports");
		else if(searchListSimulationReport_ == null || simulationReportCount == 0)
			c.o("no simulation report found");
		else
			c.o("simulation reports");
	}
	...
	protected void _pageUri(Wrap<String> c) {
		c.o("/simulation-report");
	}

	@Override
	protected void _apiUri(Wrap<String> c) {
		c.o("/api/simulation-report");
	}
	...
	protected void _pageImageUri(Wrap<String> c) {
			c.o("/png/simulation-report-999.png");
	}
	...
	protected void _classIconName(Wrap<String> c) {
			c.o("file-chart-column");
	}

	protected void _pageUriSimulationReport(Wrap<String> c) {
			c.o("/simulation-report");
	}
}
