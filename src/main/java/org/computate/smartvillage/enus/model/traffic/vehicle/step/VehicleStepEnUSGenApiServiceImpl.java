...
import org.computate.smartvillage.enus.model.traffic.vehicle.step.VehicleStepPage;
...
public class VehicleStepEnUSGenApiServiceImpl extends BaseApiServiceImpl implements VehicleStepEnUSGenApiService {
		...
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
						...
						&& !Optional.ofNullable(Optional.ofNullable(config.getString(ConfigKeys.AUTH_ROLE_READ_REQUIRED + "_VehicleStep")).orElse(config.getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_VehicleStep"))).map(v -> RoleBasedAuthorization.create(v).match(siteRequest.getUser())).orElse(false)
						) {
					String msg = String.format("401 UNAUTHORIZED user %s to %s %s", siteRequest.getUser().attributes().getJsonObject("accessToken").getString("preferred_username"), serviceRequest.getExtra().getString("method"), serviceRequest.getExtra().getString("uri"));
					eventHandler.handle(Future.succeededFuture(
						new ServiceResponse(401, "UNAUTHORIZED",
			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
					...
					try {
						...
						eventBus.request(VehicleStep.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "postVehicleStepFuture")).onSuccess(a -> {
		...
		try {
			createVehicleStep(siteRequest).onSuccess(vehicleStep -> {
				persistVehicleStep(vehicleStep, false).onSuccess(c -> {
					indexVehicleStep(vehicleStep).onSuccess(o2 -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request(VehicleStep.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "putimportVehicleStepFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
	...
	public void putimportVehicleStepFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				apiRequest.initDeepApiRequest(siteRequest);
				siteRequest.setApiRequest_(apiRequest);
				String inheritPk = Optional.ofNullable(body.getString(VehicleStep.VAR_id)).orElse(body.getString(VehicleStep.VAR_id));
	...
	public Future<VehicleStep> indexVehicleStep(VehicleStep o) {
		Promise<VehicleStep> promise = Promise.promise();
		try {
			SiteRequestEnUS siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
	...
	public String getClassApiAddress() {
		return VehicleStep.CLASS_API_ADDRESS_VehicleStep;
	}
}
